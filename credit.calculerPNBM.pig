...
codeReseau AS cdEfs,
CD_SSF_VI AS cdSFam,
MONTHS_BETWEEN(daPreDeblocage,daFinCredit) AS maturite, --(daPreDeblocage,daFinCredit)
(noCtrScr IS NOT NULL ? 1 : 0 ) AS idcEliSfh,
((tauxClient - tauxBonification) < 0.0 ? 0.0 : (tauxClient - tauxBonification) ) AS txCli,
--110 AS maxAge, --$PARAM_GLOBAL
1 AS key
;

-- pour obtenir maxAge
Contrat_age = JOIN AGE_MX_VI BY key, Contrat1 BY key;

-- pour obtenir TX_MRG_DFT
Contrat_tx_mrg_dft = JOIN  Contrat_age BY (cdSFam,cdEfs,typeTaux,maturite) LEFT OUTER,PARAM_MARGE_DFT_PLAT BY (CD_SSF_VI,CD_EFS,TY_TX,maturite);

-- pour obtenir TX_MNR_VI
Contrat_sfh = JOIN PARAM_SFH BY key, Contrat_tx_mrg_dft BY Contrat1::key;
...
cdEfs AS cdEfs,
cdSFam AS cdSFam,
Contrat1::maturite AS maturite,
txCli AS txCli,
NULL AS tci, --TODO
NULL AS spreadLiquidite, --TODO
maxAge AS maxAge,
TX_MRG_DFT AS txMargeDft,
